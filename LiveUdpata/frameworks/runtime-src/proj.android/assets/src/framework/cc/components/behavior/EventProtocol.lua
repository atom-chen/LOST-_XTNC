...
--修改库函数 根据监听名，修改监听函数
function EventProtocol:reviseEvebtListenersByeventName(eventNameTorevise,fun)
    -- body
    for eventName, listenersForEvent in pairs(self.listeners_) do
        if eventName == eventNameTorevise then
            --todo
...
function EventProtocol:removeEventListenersByTag(tagToRemove)
    ...
    for eventName, listenersForEvent in pairs(self.listeners_) do
        ...
        for handle, listener in pairs(listenersForEvent) do
            ...
            -- listener[2] = tag
            --print(handle)
            --print("--     listener: %s, handle: %s", tostring(listener[1]), tostring(listener[2]))
...
function EventProtocol:exportMethods()
    ---添加了reviseEvebtListenersByeventName
    self:exportMethods_({
        "addEventListener",
        "dispatchEvent",
        "removeEventListener",
        ...
        "reviseEvebtListenersByeventName",
    })
    return self.target_
end
